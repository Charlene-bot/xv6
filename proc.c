...
//#ifdef CS333_P4
static void assertPriority(struct proc *, int); 
...
allocproc(void)
{
  ...
  if(found){
    if(stateListRemove(&ptable.list[UNUSED], p) == -1){
      panic("failed to remove the process from unused list in allocproc()"); 
    }
...
#if defined (CS333_P3)
  if(stateListRemove(&ptable.list[EMBRYO], np) == -1){
    panic("failed to remove from EMBRYO list on successful fork()"); 
  }
  assertState(np, EMBRYO, __FUNCTION__, __LINE__); 
#endif //CS333_P3
  np->state = RUNNABLE;
...
exit(void)
{
  ...
  //Search lists for abandoned children and pass them to init
...
wait(void)
{
  ...
  for(;;){
    ...
    //scanning through the lists to check if they have kids 
    p = ptable.list[ZOMBIE].head; 
    while(p && !havekids){
      if(p->parent == curproc){
        havekids = 1;
...
scheduler(void)
{
  ...
  //Promote processes when it is time 
  ...
  for(i = 0; i < MAXPRIO; i++){
    ...
    //before jumping back to us
      if(stateListRemove(&ptable.ready[i], p) == -1)
        panic("Error failing  in scheduler() not in ready list."); 
     ...
     //Provess is done running for now 
  ...
  for(;;){
    ...
    acquire(&ptable.lock);
     ...
     for(int i = MAXPRIO; i >= 0; --i){
      ...
      if(stateListRemove(&ptable.ready[i], p) == -1)
      {
          panic("failed to removed process we will run from runnable"); 
      }
...
sched(void)
{
  ...
  #ifdef CS333_P2
    p->cpu_ticks_total = (p->cpu_ticks_total - p->cpu_ticks_in) + ticks;
  #endif //CS333_P2
...
#ifdef CS333_P3
  if(stateListRemove(&ptable.list[RUNNING], curproc) == -1)
    panic("Error: not in runnning list"); 
  assertState(curproc, RUNNING, __FUNCTION__, __LINE__); 
#endif //CS333_P3
...
sleep(void *chan, struct spinlock *lk)
{
  ...
  if(stateListRemove(&ptable.list[RUNNING], p) == -1){
    panic("failed to remove from RUNNING state in sleep()");
  }
  assertState(p, RUNNING, __FUNCTION__, __LINE__); 
  p->state = SLEEPING;
  ...
  while(p){
    if( p->chan == chan){
      if(stateListRemove(&ptable.list[SLEEPING], p) == -1){
        panic("failed to remove process from SLEEPING state in wakeup1()");      }
...
kill(int pid)
{
  ...
  //scanning through the lists to kill and wakeup if necessary
  p = ptable.list[RUNNING].head;
  while(p){
    if(p->pid == pid){
      p->killed = 1;   
  ...
  while(p){
    if(p->pid == pid){
      ...
      if(stateListRemove(&ptable.list[SLEEPING], p) == -1){
          panic("failed to remove the process from the sleeping lisr in kill()"); 
        }
...
{
  #if defined CS333_P2 || defined CS333_P4 
  //Decimal conversion for Elapsed time 
    ...
    int hundreds = (milliseconds/100);
    int tens = ((milliseconds%100)/10);
    int ones = (milliseconds%10);
    
  //Decimal conversion for CPU time 
    int cpu_sec = p->cpu_ticks_total/1000;
    int cpu_milli = p->cpu_ticks_total%1000; 
...
#ifdef CS333_P2
  cprintf("\n%d\t%s\t         %d     \t%d\t%d\t%d.%d%d%d\t%d.%d%d%d\t%s\t%d",p->pid, p->name, p->uid, p->gid, ppid, seconds, hundreds, tens, ones, cpu_sec, cpu_hun, cpu_ten, cpu_one, state_string, p->sz);
#endif //CS333_P2

#ifdef CS333_P4
  cprintf("\n%d\t%s\t         %d     \t%d\t%d\t%d\t%d.%d%d%d\t%d.%d%d%d\t%s\t%d",p->pid, p->name, p->uid, p->gid, ppid, p->priority, seconds, hundreds, tens, ones, cpu_sec, cpu_hun, cpu_ten, cpu_one, state_string, p->sz);
#endif //CS333_P4
  return;
}
#endif //CS333_P2
...
procdumpP1(struct proc *p, char *state_string) 
{
    ...
    cprintf("\n%d\t%s\t      %d.%d%d%d\t%s\t%d\t",p->pid, p->name, seconds, hundreds, tens, ones, state_string, p->sz);
  #endif //CS333_P1 
...
setpriority(int pid, int priority){
    ...
    if(pid < 0 || pid >32767 || priority < 0 || priority > MAXPRIO)
      return -1; 
